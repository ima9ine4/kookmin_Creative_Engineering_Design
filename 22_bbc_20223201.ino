...
void loop() {
  ...
  if(event_dist) {
    ...
    dist_filtered = volt_to_distance(ir_sensor_filtered(0.5, 0.5)); // Replace n with your desired value
    dist_ema = _EMA_ALPHA * dist_ema + (1.0 - _EMA_ALPHA) * dist_filtered;

    // bang bang control
    if(dist_target < dist_ema) {//bring ball farther // Replace 1 with a proper test expression using dist_target and dist_ema.
      duty_target = _DUTY_MIN;  // Complete this line using _BANGBANG_RANGE.
      digitalWrite(PIN_LED, 1); //LED OFF
    }
    else { //bring ball closer
...
{
  return 762 + -3.39*a_value + 5.31E-03*a_value*a_value + - 2.93E-06*a_value*a_value*a_value;
}
